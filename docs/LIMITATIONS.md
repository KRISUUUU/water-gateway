...
6. **Long-frame handling is conservative in polling mode.** Frames that exceed the
   current safe FIFO drain window are dropped explicitly (not truncated) and reported
   via `frames_dropped_too_long` / `frames_incomplete` counters.
7. **Detected meter identity is best-effort.** Gateway identity uses observed
...
1. **2.4 GHz WiFi only.** ESP32 does not support 5 GHz WiFi.
2. **No Ethernet.** Only WiFi STA mode is supported for network connectivity.
3. **HTTP only (no HTTPS) for web panel.** TLS for the web panel is not
   enabled due to ESP32 memory constraints and certificate provisioning complexity.
4. **IPv4 only.** IPv6 support is not tested or configured.

## MQTT

1. **QoS 0 by default.** Messages may be lost during broker disconnect.
...
1. **Single NVS blob.** The entire config is stored as one NVS blob. Partial
    updates rewrite the whole blob. This is atomic but slower than per-field storage.
2. **No config encryption by default.** NVS contents are readable if flash is
    extracted. NVS encryption is available but requires key management during
    manufacturing.
3. **No multi-user / RBAC.** There is a single admin account. No read-only
    user role exists.

## OTA

1. **1.5 MB OTA firmware size limit.** Constrained by current 4 MB partition layout.
2. **No firmware signing.** Secure Boot v2 is available in ESP-IDF but not
    enabled by default due to key management requirements.
3. **No delta/incremental OTA.** Full firmware image is required for each update.

## Web Panel

1. **No WebSocket.** Data updates use polling (fetch-based), not push.
2. **No offline capability.** The panel requires network access to the device.
3. **Vanilla JS only.** No component framework; complex UI extensions may
    require significant refactoring.

## General

1. **No persistent logs.** Log buffer is RAM-only and lost on reboot.
    Flash-based persistent logging is a future enhancement.
2. **32-bit counters.** Diagnostic counters are uint32_t and will wrap after
    ~4 billion events. Wrap is handled gracefully but may confuse monitoring
    if not accounted for.
3. **Clock accuracy.** NTP sync is periodic; between syncs, the ESP32 RTC
