...
3. WiFi AP starts with SSID `WMBus-GW-Setup`, protected with WPA2-PSK
...
- The AP uses **WPA2-PSK** with a device-unique password derived from the lower 4 bytes of the device MAC address (e.g. `A1B2C3D4`)
- The AP password is printed to the **UART serial console** at boot — connect via serial monitor to read it
- The password is NOT stored in config, support bundles, or remote log channels (UART only)
