...
std::string payload_status_online(const char* firmware_version, const char* ip_address,
    ...
    std::snprintf(buf, sizeof(buf),
                  ...
                  json_escape(firmware_version).c_str(), json_escape(ip_address).c_str(),
                  json_escape(hostname).c_str(), static_cast<unsigned long>(uptime_s),
...
std::string payload_telemetry(uint32_t uptime_s, uint32_t free_heap_bytes,
    ...
    std::snprintf(buf, sizeof(buf),
                  ...
                  (unsigned long)uptime_s, (unsigned long)free_heap_bytes,
                  (unsigned long)min_free_heap_bytes, (int)wifi_rssi_dbm,
                  json_escape(mqtt_state).c_str(), json_escape(radio_state).c_str(),
                  (unsigned long)frames_received, (unsigned long)frames_published,
                  (unsigned long)frames_duplicate, (unsigned long)frames_crc_fail,
                  (unsigned long)mqtt_publishes, (unsigned long)mqtt_failures,
...
std::string payload_event(const char* event_type, const char* severity, const char* message,
    ...
    std::snprintf(buf, sizeof(buf),
                  ...
                  json_escape(event_type).c_str(), json_escape(severity).c_str(),
                  json_escape(message).c_str(), json_escape(timestamp).c_str());
    return std::string(buf);
}
...
std::string payload_raw_frame(const char* raw_hex, uint16_t frame_length, int8_t rssi_dbm,
    ...
    std::snprintf(buf, sizeof(buf),
                  ...
                  json_escape(raw_hex).c_str(), (unsigned)frame_length, (int)rssi_dbm,
                  (unsigned)lqi, crc_ok ? "true" : "false", (unsigned)manufacturer_id,
                  (unsigned long)device_id, json_escape(meter_key).c_str(),
                  json_escape(timestamp).c_str(), (unsigned long)rx_count);
    return std::string(buf);
}
